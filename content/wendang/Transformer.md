...
- 它是目前几乎所有大语言模型（ChatGPT、Llama、Qwen、DeepSeek 等）的**核心骨架**。
...
- BERT 是 Google 在 2018 年发布的**预训练语言模型**，它**使用了 Transformer 中的 Encoder（编码器）部分**。

- BERT 的最大创新是**双向理解**：它能同时看一个词的左边和右边上下文（而之前的模型大多只能看一边）。
...
在 BERT 处理文本时，会在**输入文本的最前面**自动加上一个特殊的 Token：[CLS]。
...
因此，在做**文本分类、情感分析、相似度计算**等任务时，人们通常**直接取出 [CLS] 位置的向量**来代表整个文本块，而不需要再对所有 Token 做平均。
